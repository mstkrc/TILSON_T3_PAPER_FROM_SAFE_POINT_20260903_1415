...
from ..components import display_button, kv_list, panel, progress, scroll_container, table
...
def _services() -> str:
    ...
    return panel(
        "HİZMET DURUMLARI",
        scroll_container(table(("", "HİZMET", "DURUM", "YANIT SÜRESİ", "SON KONTROL"), rows, "dense"), "Hizmet durumları", "group2-scroll"),
        "fill",
    )
...
def _event_log() -> str:
    ...
    return panel(
        "SİSTEM OLAY GÜNLÜĞÜ",
        scroll_container(table(("ZAMAN", "SEVİYE", "KAYNAK", "OLAY", "AÇIKLAMA"), rows, "dense"), "Sistem olay günlüğü", "group2-scroll"),
        "fill",
        "SON 20",
    )
...
def render_system_health_screen(model=None) -> str:
    ...
    body = _top_metrics() + f'<div class="group2-health-main"><div class="stack fill">{middle}{lower}</div>{_right_rail()}</div>'
