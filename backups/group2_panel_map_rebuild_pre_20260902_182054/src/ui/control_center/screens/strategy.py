...
def _parameter_cards() -> str:
    ...
    return f'<div class="screen-grid group2-strategy-top">{t3}{dmi}{volume}{risk}</div>'
...
def _signal_flow() -> str:
    ...
    return panel("SİNYAL ÜRETİM KURALLARI", f'<div class="group2-flow">{long_rules}{chain}{short_rules}</div>', "fill", "CLOSED CANDLE")
...
def _signal_states() -> str:
    ...
    return panel("SİNYAL DURUMLARI", content, "fill", "AÇIKLANABİLİR KARAR")
...
def render_strategy_screen(model=None) -> str:
    lower = f'<div class="group2-strategy-main"><div class="group2-strategy-content">{_signal_flow()}{_signal_states()}{_change_rule()}</div>{_right_rail()}</div>'
    body = _management_bar() + _parameter_cards() + lower
