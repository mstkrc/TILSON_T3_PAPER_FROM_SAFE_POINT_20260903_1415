...
from ..components import bar_chart, display_button, kv_list, line_chart, panel, progress, scroll_container, table
...
def _risk_events() -> str:
    ...
    return panel(
        "RİSK OLAYLARI",
        scroll_container(table(("ZAMAN", "OLAY", "SEVİYE", "AÇIKLAMA"), rows, "dense"), "Risk olayları", "group2-scroll"),
        "fill",
        "SON 20",
    )
...
def render_risk_screen(model=None) -> str:
    ...
    body = _top_metrics() + f'<div class="group2-risk-main"><div class="stack fill">{middle}{lower}</div>{_right_rail()}</div>'
