...
def _management_bar() -> str:
    return (
        '<section class="cc-panel group2-strategy-management">'
        '<header class="group2-management-header">'
        '<div class="group2-management-copy"><h2>STRATEJİ YÖNETİMİ</h2>'
        '<p>Tüm parametreler sadece yeni giriş adaylarını etkiler. Açık pozisyonlar etkilenmez.</p></div>'
        '<div class="group2-management-actions"><label>Strateji Profili</label>'
        '<span class="display-field"><b>EMA Model Trade v1.6⌄</b></span>'
...
def render_strategy_screen(model=None) -> str:
    lower = f'<div class="group2-strategy-content">{_signal_flow()}{_signal_states()}{_change_rule()}</div>'
    body = (
        '<div class="group2-panel-map group2-strategy-map" data-panel-map="07_STRATEJI">'
        + '<div class="group2-strategy-layout" data-layout="isolated-height-budget">'
        + '<div class="group2-strategy-left">' + _management_bar() + _parameter_cards() + lower + '</div>'
        + _right_rail() + "</div></div>"
    )
