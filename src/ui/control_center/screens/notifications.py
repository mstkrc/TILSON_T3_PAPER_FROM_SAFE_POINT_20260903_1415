...
def render_notifications_screen(model=None) -> str:
    metrics = '<div class="metric-grid grid-5 top-band">' + ''.join((metric_card("Bugün", "42", "bildirim"), metric_card("Başarılı", "40", "%95.2"), metric_card("Kuyruk", "2", "bekliyor"), metric_card("Reddedilen", "3", "unauthorized"), metric_card("Ağ Bağlantısı", "YOK", "security model only"))) + '</div>'
    ...
    whitelist = panel("WHITELIST / ROLLER", table(("Kimlik", "Rol", "Yetki"), (("user-001", "OWNER", "READ ONLY"), ("user-ops", "OPERATOR", "STATUS / PNL"), ("unknown", "NONE", "REJECT")), "dense"), "compact")
    ...
    queued = panel("KUYRUKTA", scroll_container(table(("Sıra", "Olay", "Plan", "Durum"), (("1", "Health summary", "19:45", "BEKLEYEN"), ("2", "Ledger summary", "20:00", "BEKLEYEN")), "dense"), "Bildirim kuyruğu"), "event-panel")
    ...
    actions = panel("HIZLI AKSİYONLAR", action_stack((("TEST BİLDİRİMİ · DISABLED", "neutral"), ("AUDIT GÖRÜNÜMÜ", "info"), ("PANIC ONAY AKIŞI · DISPLAY", "danger"))), "compact")
    mute = panel("SESSİZE ALMA KURALLARI", kv_list((("Info tekrar", "5 dk"), ("Warning tekrar", "1 dk"), ("Critical", "ASLA"))), "compact")
    body = metrics + f'<div class="screen-grid with-rail fill"><div class="stack fill"><div class="screen-grid grid-3">{channels}{rules}{whitelist}</div>{latest}{queued}</div><aside class="stack fill">{controls}{actions}{mute}</aside></div>'
